...
### Goal
Use the SNS push notifications generated by AWS to update to update the Sat-API with new Sentinel 2 metadata. This would be better than the current working solution of scraping the bucket daily for new additions to the repository, because it would be more efficient and result in faster updates. New products appear to come in at a rate of about 5 per minute (average).
...
- Run a lambda function each time a new message is received.
  ...
  - Update the elasticsearch index with the new metadata so it can be served via sat-api.
    - Most of this logic was inspired by the (now deprecated) [metadata](https://github.com/sat-utils/sentinel2-metadata) library.
...
### Potential hiccups for new contributors

#### Steps for packaging python lambda deployment package 
...
#### Errors Upon Subscribing to the Sentinel2 SNS 
When you try to add a new subscription to the Sentinel2 SNS service, you may find that there are issues regarding your permissions. These warnings/errors persist to modifying or deleting your subscription as well. As far as I understand, it is not possible to remove your subscription due to these errors. However, fear not -- although your subscription may have triggered a warning it was likely still created. Check your Lambda console to find out. This happened on both DS and personal AWS accounts and in both cases, the subscription was successful despite the warning. 

#### What elastic search domain? 
The dummy ES domain mentioned above was created using my personal credentials (since I didn't have access to the DS ES service). Another can be easily created using the default settings. Set the environment variable in the lambas console to point to the domain you've just created. It might take a bit of time for the new domain to register. You can also set the port number of your ES instance if you want, also in the environment variables section. 

#### Metadata.csv?
At some point, it appears that the `sat-api` is working from CSVs that are subsequently ingested into AWS. See [here](https://github.com/sat-utils/sat-api/blob/develop/lib/metadata.js#L16). Not sure about this, giving cause rise to the thought that `sat-api` may expect more/different data than what is provided by the updater. 
