...
- Run a lambda function each time a new message is received.
  ...
  - Update the elasticsearch index with the new metadata so it can be served via sat-api.
    ...
    - I focused on making sure that the ES domain was being populated with the metadata (IE, there might be a gap between what the Sat-API is expecting, and what the updater is giving it).
    - I started running the lambda->ES after lunch on Friday, and as of 3:30, it has been updated with 447 new scenes.
